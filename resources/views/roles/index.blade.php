        ...
        <div class="content-wrapper">
          ...
          <div class="card">
            ...
            <div class="row mb-3">
                ...
                <div class="col-1">
                @can('create-role')
                    <a href="{{ route('roles.create') }}" class="btn btn-primary"><i class="fa fa-plus-circle"></i></a>
                @endcan
                </div>
            </div>
              <div class="row">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <form id="delete-form-{{ $role->id }}" action="{{ route('roles.destroy', $role->id) }}" method="post">
                            ...
                            <a href="{{ route('roles.show', $role->id) }}" class="btn btn-outline-primary"  title="Show"><i class="fa fa-eye"></i></a>

                            @if ($role->name!='Super Admin')
                                @can('edit-role')
                                    <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-outline-warning" title="Edit"><i class="fas fa-pencil-alt"></i></a>   
                                @endcan

                                @can('delete-role')
                                    @if ($role->name!=Auth::user()->hasRole($role->name))
                                    <button type="button" class="btn btn-outline-danger delete-btn" title="Delete" data-id="{{ $role->id }}">
